...
class SettingsPanel extends StatefulWidget {
  ...
  const SettingsPanel({super.key, this.controller, this.asRoute = false});
...
class _SettingsPanelState extends State<SettingsPanel> {
  ...
  // Helper para asegurar índices enteros válidos en _resolutions
  int _boundedIndex(int i) {
    if (_resolutions.isEmpty) return 0;
    return max(0, min(i, _resolutions.length - 1));
  }
  ...
  bool _indexHasVisibleWidget(int idx) {
    ...
    try {
      final size = render.paintBounds.size;
  ...
  int _findNextVisibleIndex(int start, int step) {
    var i = start + step;
    while (i >= 0 && i <= _maxFocus) {
      if (_indexHasVisibleWidget(i)) return i;
      i += step;
    }
    return start; // no visible next found -> stay
  }
        ...
        if (_resolutions.isEmpty) {
          ...
          if (idx >= 0) {
            ...
            _pendingWindowResolutionIndex =
                _boundedIndex(_resolutions.indexOf(closest));
          }
        } else {
          _pendingWindowResolutionIndex = 0;
  ...
  void _onUp() {
    ...
    setState(() {
      final next = _findNextVisibleIndex(_focusIndex, -1);
      _focusIndex = next;
    });
    _ensureFocusVisible();
  }
  ...
  Future<void> _savePendingAndClose() async {
    ...
    try {
      ...
      if (_resolutions.isNotEmpty) {
        _pendingWindowSize =
            _resolutions[_boundedIndex(_pendingWindowResolutionIndex)];
      }
      ...
      // Apply window settings (may persist window related values)
      final wasFullscreen = s.isFullscreen.value;
      await s.applyWindowSettings(
          fullscreen: _pendingFullscreen,
          size: _pendingWindowSize,
          persist: true);

      // Persist all settings (audio, controls, etc.) so they take effect globally
      try {
        await s.save();
      } catch (e) {
      ...
      // Pop respetando asRoute: si se abrió como ruta -> pop una vez, si no -> popUntil first
      try {
        // if we just toggled fullscreen off, allow the window to stabilize first
        if (wasFullscreen && !_pendingFullscreen) {
  ...
  Widget build(BuildContext context) {
    return RawKeyboardListener(
      ...
      child: GamepadListener(
        // Mapeo de entrada de mando a las acciones del panel.
        onLeft: _onLeft,
        onRight: _onRight,
        onUp: _onUp,
        onDown: _onDown,
        ...
        onBack: _onBack,
        // Triángulo (botón Y) -> aplicar cambios (atajo rápido con mando)
        onTriangle: _applyFromController,
        child: Scaffold(
