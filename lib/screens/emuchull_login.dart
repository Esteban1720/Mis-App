...
class _EmuChullLoginScreenState extends State<EmuChullLoginScreen>
  ...
  Widget build(BuildContext context) {
    return RawKeyboardListener(
      ...
      onKey: (RawKeyEvent ev) async {
        if (ev is RawKeyDownEvent) {
          final k = ev.logicalKey;
          if (k == LogicalKeyboardKey.f11) {
            try {
              // Si el fondo de video está inicializado, pausar antes de
              // alternar fullscreen para evitar problemas con la superficie
              // nativa de libVLC que pueden ocultar/congelar la ventana.
              if (_videoInitialized && _vlcPlayer != null) {
                try {
                  _vlcPlayer!.pause();
                } catch (_) {}
                await InputService.instance.toggleFullscreen();
